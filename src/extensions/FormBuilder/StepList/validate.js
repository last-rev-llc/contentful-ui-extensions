...
import { filter, flatten, identity, indexBy, isEmpty, map, merge, negate, pipe, prop } from 'lodash/fp';
...
const errorLevelsMapping = {
  [errors.CONFLICT_NAME]: errorLevels.WARN,
  [errors.UNDEFINED_NAME]: errorLevels.ERROR,
  [errors.UNDEFINED_TYPE]: errorLevels.ERROR,
  [errors.UNDEFINED_ID]: errorLevels.ERROR
};
...
function validateField(field) {
  ...
  if (!id) {
    toReturn.push({ type: errors.UNDEFINED_ID, id: null, name });
  }

  // All fields must have a name
  if (!field.name) {
    toReturn.push({ type: errors.UNDEFINED_NAME, id });
  }

  // All fields must have a type
  if (!field.type) {
    toReturn.push({ type: errors.UNDEFINED_TYPE, id });
  }
