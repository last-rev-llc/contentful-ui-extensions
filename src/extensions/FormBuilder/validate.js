...
import { filter, flatten, identity, indexBy, isEmpty, map, merge, negate, pipe, pickBy, prop } from 'lodash/fp';
...
const errorLevelsMapping = {
  [errorTypes.CONFLICT_NAME]: errorLevels.ERROR,
...
function validateFieldNames(steps) {
  ...
  return Object.entries(fieldNameCounts)
    ...
    .map(([name, { ids }]) => ids.map((id) => ({ type: errorTypes.CONFLICT_NAME, id, name })));
}
...
export const onlyErrors = pickBy(({ level }) => level === errorLevels.ERROR);

export const onlyWarnings = pickBy(({ level }) => level === errorLevels.WARN);
