...
const renderComponent = (sdk = defaultSdk) => render(<RecipeIngredients sdk={sdk} />);
...
describe('<RecipeIngredients />', () => {
  ...
  describe('IngredientList', () => {
    ...
    test('Table Row Step renders correctly', () => {
      ...
      expect(getAllByTestId('Ingredients-Table-Cell-Step').length).toBe(
        mockSdkList.field.getValue().ingredients.length
      );
    });
    test('Table Row Ingredient renders correctly', () => {
      const { getAllByTestId } = renderComponent(mockSdkList);
      expect(getAllByTestId('Ingredients-Table-Cell-Ingredient').length).toBe(
        mockSdkList.field.getValue().ingredients.length
      );
    });
    test('Table Row ImperialQuantity renders correctly', () => {
      const { getAllByTestId } = renderComponent(mockSdkList);
      expect(getAllByTestId('Ingredients-Table-Cell-ImperialQuantity').length).toBe(
        mockSdkList.field.getValue().ingredients.length
      );
    });
    test('Table Row MetricQuantity renders correctly', () => {
      const { getAllByTestId } = renderComponent(mockSdkList);
      expect(getAllByTestId('Ingredients-Table-Cell-MetricQuantity').length).toBe(
        mockSdkList.field.getValue().ingredients.length
      );
    });
    test('Table Row Actions renders correctly', () => {
      const { getAllByTestId } = renderComponent(mockSdkList);
      expect(getAllByTestId('Ingredients-Table-Cell-Actions').length).toBe(
        mockSdkList.field.getValue().ingredients.length
      );
    });
  });
