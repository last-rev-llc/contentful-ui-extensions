...
const getFields = (field) => {
  ...
  switch (field.type) {
    case fieldTypes.richText:
      result = getTextDiff({
        id: 0,
        oldText: _.get(field, 'content.oldValue', ''),
        newText: _.get(field, 'content.currentValue', ''),
    ...
    case fieldTypes.symbol:
      result = getTextDiff({
        id: 0,
        oldText: _.escape(_.get(field, 'oldValue', '')),
        newText: _.escape(_.get(field, 'currentValue', '')),
        fieldType: field.type,
      });
      break;
    ...
    case fieldTypes.text:
      result = getTextDiff({
        id: 0,
        oldText: _.get(field, 'oldValue', ''),
        newText: _.get(field, 'currentValue', ''),
        fieldType: field.type,
      });
      break;
    ...
    case fieldTypes.array:
      if (field.arrayType === fieldTypes.symbol) {
        result = getTextDiff({
          id: 0,
          oldText: getArrayValue(_.get(field, 'oldValue', [])),
          newText: getArrayValue(_.get(field, 'currentValue', [])),
          fieldType: field.type,
        });
      }
      break;
    ...
    case fieldTypes.link:
      ...
      if (field.linkType === linkTypes.asset) {
        result = getTextDiff({
          id: 0,
          oldText: createAssetHtml(_.get(field, 'oldValue')),
          newText: createAssetHtml(_.get(field, 'currentValue')),
          fieldType: field.type,
        });
      } else {
        result = getTextDiff({
