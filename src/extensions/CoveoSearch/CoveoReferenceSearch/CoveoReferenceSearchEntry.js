...
function CoveoReferenceSearchEntry({ space, contentId, removeHandler }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { displayField, name } = await space.getContentType(get(entry, 'sys.contentType.sys.id'));
    ...
    <EntityListItem
      ...
      dropdownListElements={
        <DropdownList>
          ...
          <DropdownListItem onClick={() => removeHandler(contentId)}>Remove</DropdownListItem>
        </DropdownList>
      }
    />
  );
