...
const Seo = ({ sdk }) => {
  ...
  const removeFieldProperty = (metaTagName) => {
    sdk.field.setValue(omit(seoObject, metaTagName)).then((response) => {
      setSeoObject(response);
    });
  };
  ...
  const renderTabs = () => {
    return (
      <Tabs withDivider testId="Seo-tabs">
        <Tab
          id="preview"
          selected={selected === 'preview'}
          onSelect={() => setSelected('preview')}
          testId="Seo-tab-preview">
          Preview
        </Tab>
        <Tab
          id="general"
  ...
  const renderPreview = () => {
    return (
      <TabPanel id="preview" className="tab-panel" testId="Seo-tabpanel-preview">
        <div className="search-preview">
          <a href="#">
            <h3 className="" data-test-id="Seo-tabpanel-preview-title">
              {get(seoObject, 'title.value', 'Please enter a page title')}
            </h3>
            <br />
            <div className="cite">
              <cite className="" data-test-id="Seo-tabpanel-preview-cite">
                {get(seoObject, 'canonical.value', 'https://www.example.com › path › slug')}
              </cite>
            </div>
            <div className="description" data-test-id="Seo-tabpanel-preview-description">
              {get(
                seoObject,
                'description.value',
  ...
  const renderGeneralTab = () => {
    return (
      <TabPanel id="general" className="tab-panel" testId="Seo-tabpanel-general">
        <TextField
          id="title"
          name="title"
        ...
        {isSearchVisible() ? null : (
          <Note noteType="warning" testId="Seo-tabpanel-general-noindex-note">
            Your content is not being indexed
          </Note>
        )}
      </TabPanel>
    );
  ...
  const renderFacebookTab = () => {
    ...
    return (
      <TabPanel id="facebook" className="tab-panel" testId="Seo-tabpanel-facebook">
        <TextField
          id="og:title"
          name="og:title"
        ...
        <TextField
          ...
          className="fieldset"
        />
        <FormLabel className="fieldset" htmlFor="og:image">
          Post Image
        </FormLabel>
        ...
        />
        <HelpText>
          Use images that are at least 1200 x 630 pixels for the best display on high resolution devices. At the
          minimum, you should use images that are 600 x 315 pixels to display link page posts with larger images.
        </HelpText>
      </TabPanel>
    );
  };
  ...
  const renderTwitterTab = () => {
    return (
      <TabPanel id="twitter" className="tab-panel" testId="Seo-tabpanel-twitter">
        <TextField
          id="twitter:title"
          name="twitter:title"
          labelText="Post Title"
        ...
        <TextField
          ...
          className="fieldset"
        />
        <FormLabel className="fieldset" htmlFor="twitter:image">
          Post Image
        </FormLabel>
  ...
  return (
    <>
      {renderTabs()}
      {selected === 'preview' && renderPreview()}
      {selected === 'general' && renderGeneralTab()}
      {selected === 'facebook' && renderFacebookTab()}
      {selected === 'twitter' && renderTwitterTab()}
    </>
  );
};
