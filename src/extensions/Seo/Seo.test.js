...
describe('<Seo />', () => {
  describe('initialize with useEffect()', () => {
    test('preview tab is selected by default', () => {
      ...
      expect(getByTestId('Seo-tab-preview').getAttribute('aria-selected')).toBe('true');
  ...
  describe('renderTabs()', () => {
    test('four tabs are present', () => {
      const { getByTestId } = render(<Seo sdk={sdk} />);
      expect(getByTestId('Seo-tabs').children.length).toBe(4);
    });
  ...
  describe('renderPreview()', () => {
    test('default text is shown when no seo object is present', () => {
      ...
      expect(getByTestId('Seo-tabpanel-preview-title').textContent).toBe('Please enter a page title');
      expect(getByTestId('Seo-tabpanel-preview-description').textContent).toBe(
        'Please enter a meta description that is between 100 and 250 characters long'
      );
    });
    test('pageTitle should equal the value of the title property', () => {
      const { getByTestId } = render(<Seo sdk={sdk} />);
      expect(getByTestId('Seo-tabpanel-preview-title').textContent).toBe(sdk.field.getValue().title.value);
    });
    test('description equals the value of description property', () => {
      const { getByTestId } = render(<Seo sdk={sdk} />);
      expect(getByTestId('Seo-tabpanel-preview-description').textContent).toBe(sdk.field.getValue().description.value);
    });
  ...
  describe('renderGeneralTab()', () => {
    test('input field values default to object property values', () => {
      const { getByTestId } = render(<Seo sdk={sdk} />);
      fireEvent.click(getByTestId('Seo-tab-general'));
      expect(getByTestId('Seo-tabpanel-general-title').value).toBe(sdk.field.getValue().title.value);
      expect(getByTestId('Seo-tabpanel-general-description').value).toBe(sdk.field.getValue().description.value);
      expect(getByTestId('Seo-tabpanel-general-keywords').value).toBe(sdk.field.getValue().keywords.value);
      expect(getByTestId('Seo-tabpanel-general-robots-true').checked).toBeTruthy();
    });
    ...
    test('note warning that the content is not being indexed shown when robots value is noindex, nofollow', () => {
      ...
      fireEvent.click(getByTestId('Seo-tab-general'));
      expect(getByTestId('Seo-tabpanel-general-robots-false').checked).toBeTruthy();
    });
  ...
  describe('renderFacebookTab()', () => {
    test('fields should default to values in the seo object', async () => {
      ...
      fireEvent.click(getByTestId('Seo-tab-facebook'));
      await wait();
      expect(getByTestId('Seo-tabpanel-og:title').value).toBe(sdk.field.getValue()['og:title'].value);
      expect(getByTestId('Seo-tabpanel-og:description').value).toBe(sdk.field.getValue()['og:description'].value);
    });
    ...
    test('asset card shown when value for og:image is valid url', async () => {
      ...
      expect(
        getByTestId('SingleAssetWithButton-AssetCard')
          .getElementsByTagName('img')[0]
          .getAttribute('src')
      ).toBe(_.get(mockContentfulAsset.success, `fields.file.${sdk.field.locale}.url`));
      expect(queryByTestId('SingleAssetWithButton-Button')).toBeNull();
    });
    ...
    test('should default to button if no value for og:image is present', async () => {
      ...
      fireEvent.click(getByTestId('Seo-tab-facebook'));
      await wait();
      expect(queryByTestId('SingleAssetWithButton-AssetCard')).toBeNull();
      expect(queryByTestId('SingleAssetWithButton-Button')).toBeTruthy();
    });
  });
  ...
  describe('renderTwitterTab()', () => {
    ...
    test('should render twitter field values if present', async () => {
      ...
      fireEvent.click(getByTestId('Seo-tab-twitter'));
      await wait();
      expect(getByTestId('Seo-tabpanel-twitter:title').value).toBe(sdk.field.getValue()['twitter:title'].value);
      expect(getByTestId('Seo-tabpanel-twitter:description').value).toBe(
        sdk.field.getValue()['twitter:description'].value
      );
    });
