    ...
    return (
      <SelectField
        ...
        {contentTypeDropdownOptions.map((contentType) => {
          return (
            <Option value={contentType.sys.id} key={contentType.sys.id} testId="SeoConfig-option-contentType">
              {contentType.name}
            </Option>
          );
        })}
  ...
  renderDefaultFieldConfig = (fieldPath, contentTypeFields, fieldType, contentTypeId) => {
    ...
    return (
      <Select
        ...
        <Option value="0" key="0" testId="SeoConfig-option-fields-default">
          --
        </Option>
        {validFields.map((field) => {
          return (
            // TODO: Show a link to edit the content type if no valid one exists for the dropdown
            <Option value={field.id} key={field.id} testId="SeoConfig-option-fields">
              {field.name}
            </Option>
          );
        })}
    ...
    if (!isEmpty(contentTypeConfig) && get(contentTypeConfig, contentType.sys.id)) {
      ...
      return (
        <TableRow key={Math.random()} testId="SeoConfig-tablerow-contentType">
          ...
          <TableCell testId="SeoConfig-tablecell-contentType-seo">
            {this.renderDefaultFieldConfig('fieldId', contentType.fields, 'Object', contentType.sys.id)}
          </TableCell>
          <TableCell testId="SeoConfig-tablecell-contentType-noindex">yes/no</TableCell>
          <TableCell testId="SeoConfig-tablecell-contentType-defaultPageTitleField">
            {this.renderDefaultFieldConfig(
              'settings.defaultPageTitleField',
              contentType.fields,
              'Symbol',
              contentType.sys.id
            )}
          </TableCell>
          <TableCell testId="SeoConfig-tablecell-contentType-defaultDescriptionField">
            {this.renderDefaultFieldConfig(
              'settings.defaultDescriptionField',
              contentType.fields,
              'Symbol',
              contentType.sys.id
            )}
          </TableCell>
          <TableCell testId="SeoConfig-tablecell-contentType-defaultCanonicalField">
            {this.renderDefaultFieldConfig(
              'settings.defaultCanonicalField',
              contentType.fields,
              'Symbol',
              contentType.sys.id
            )}
          </TableCell>
          <TableCell testId="SeoConfig-tablecell-contentType-defaultSocialImageField">
            {this.renderDefaultFieldConfig(
              'settings.defaultSocialImageField',
              contentType.fields,
              'Asset',
              contentType.sys.id
            )}
      ...
      <Table className="fieldset" testId="SeoConfig-table-contentType">
        <TableHead>
          <TableRow testId="SeoConfig-tablehead-contentType">
            ...
            <TableCell testId="SeoConfig-tablehead-contentType-defaultCanonicalField">Canonical URL Field</TableCell>
        ...
        <TableBody testId="SeoConfig-tablebody-contentType">
          {isEmpty(contentTypeConfig)
            ...
            : keys(contentTypeConfig).map((contentTypeId) => {
                return this.renderContentTypeConfigRow(find(contentTypes, ['sys.id', contentTypeId]));
              })}
        </TableBody>
      </Table>
    );
