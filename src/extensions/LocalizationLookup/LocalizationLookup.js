...
export const renderError = (error, message, position) => {
  return error ? (
    <div className="alert alert-danger" data-test-id={`error-${position}`}>
      {message}
    </div>
  ) : null;
};
...
export const renderFieldProperty = (nameField, valueField, onNameChange, onValueChange, readOnly, position) => {
  return (
    <div className="my-2">
      <div className="d-inline-block">{renderNameField(nameField, onNameChange, readOnly, position)}</div>
      <span className="d-inline-block mx-3">:</span>
      <div className="d-inline-block">{renderValueField(valueField, onValueChange, position)}</div>
    </div>
  );
};
...
const LocalizationLookup = ({ sdk }) => {
  ...
  const renderFieldFactory = () => {
    return (
      <div>
        ...
        <div className="d-inline-block ml-3">{renderButton('+', 'positive', () => addProperty(), 'factory')}</div>
        {renderFactoryErrors()}
      </div>
    );
  };
  ...
  const renderFieldItem = (nameIn, valueIn, position) => {
    ...
    return disable ? (
      <div>
        ...
        <div className="d-inline-block ml-3">{renderButton('Edit', 'primary', onEditClick, position)}</div>
        <div className="d-inline-block ml-1">{renderButton('-', 'negative', onDelete, position)}</div>
      </div>
    ) : (
      <div>
        <div className="d-inline-block">
        ...
        <div className="d-inline-block ml-3">{renderButton('Save', 'primary', onEditSave, position)}</div>
        <div className="d-inline-block ml-1">{renderButton('Cancel', 'muted', onCancel, position)}</div>
  ...
  return (
    <>
      <div>{renderFieldFactory()}</div>
      <div className="mt-4">{renderFieldList()}</div>
    </>
  );
};
