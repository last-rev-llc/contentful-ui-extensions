  ...
  return (
    <TableRow>
      <TableCell className="operatingHours__dateTableCell">
        <TextInput width="small" value={format(new Date(value.date), 'MM/d/y')} disabled />
      </TableCell>
      ...
      <TableCell>{value.timeRange.join(' - ')}</TableCell>
      <TableCell className="operatingHours__actionsTableCell">
        <CardActions testId={`${id}-actions`}>
          <DropdownList>
            <DropdownListItem onClick={clickEdit} testId={`${id}-edit`}>
              Edit
            </DropdownListItem>
            <DropdownListItem onClick={clickRemove} testId={`${id}-remove`}>
              Remove
            </DropdownListItem>
          </DropdownList>
        </CardActions>
