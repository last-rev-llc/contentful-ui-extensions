...
import { Paragraph, Table, TableHead, TableBody, TableRow, TableCell } from '@contentful/forma-36-react-components';
...
function OverrideDaysTable({ overrideDays, addOverrideDay, editOverrideDay, deleteOverrideDay }) {
  ...
  return (
    <>
      <Table className="operatingHours__table" testId="overrideDaysTable">
        <TableHead>
          <TableRow>
        ...
        <TableBody testId="overrideDaysTableBody">
          {(!overrideDays || overrideDays.length === 0) && (
            <TableRow>
              <td className="operatingHours__emptyTableRow" colSpan="5">
                <Paragraph>There are no special dates registered yet. Fill the form below to add one.</Paragraph>
              </td>
            </TableRow>
          )}
...
OverrideDaysTable.propTypes = {
  overrideDays: PropTypes.arrayOf(
    PropTypes.shape({
