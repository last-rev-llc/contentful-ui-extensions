...
function FriendlyLabelsTable({ friendlyLabels, addRow, editRow, deleteRow }) {
  ...
  return (
    <>
      <Table className="operatingHours__table" testId="friendlyLabelsTable">
        <TableHead>
          <TableRow>
        ...
        <TableBody testId="friendlyLabelsTableBody">
          {(!friendlyLabels || friendlyLabels.length === 0) && (
            <TableRow>
              <td className="operatingHours__emptyTableRow" colSpan="3">
                <Paragraph>There are no friendly labels registered yet. Fill the form below to add one.</Paragraph>
              </td>
            </TableRow>
          )}
            ...
            <TableRow key={friendlyLabel.description}>
              ...
              <TableCell className="operatingHours__actionsTableCell">
                <CardActions testId={`friendlyLabel-${index}-actions`}>
                  <DropdownList>
                    <DropdownListItem onClick={() => enterRowEditMode(index)} testId={`friendlyLabel-${index}-edit`}>
                      Edit
                    </DropdownListItem>
                    <DropdownListItem onClick={() => deleteRow(index)} testId={`friendlyLabel-${index}-remove`}>
                      Remove
                    </DropdownListItem>
                  </DropdownList>
                </CardActions>
...
FriendlyLabelsTable.propTypes = {
  friendlyLabels: PropTypes.arrayOf(
    PropTypes.shape({
