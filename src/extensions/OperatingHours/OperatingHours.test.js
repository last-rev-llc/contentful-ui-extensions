...
jest.mock('../../shared/components/TimezoneDropdown', () => ({ id, value, onChange }) => {
  return (
    <select data-test-id={id} value={value} onChange={onChange}>
...
describe('<OperatingHours />', () => {
  ...
  describe('Regular Hours tab', () => {
    ...
    test('timezone, isClosed and time range fields should be enabled', () => {
      ...
      const isClosedCheckbox = getByTestId('OperatingHours-RegularHours-Monday-isClosed').querySelector(
        'input[type="checkbox"]'
      );
  ...
  describe('Special Dates tab', () => {
    ...
    test('remove a special date', async () => {
      ...
      expect(overrideSdk.field.setValue.mock.results[0].value.overrideDays.map((od) => od.date)).not.toContain(
        '12/26/2020'
      );
    });
  });
  ...
  describe('Friendly Labels tab', () => {
      ...
      const overrideSdk = {
        ...
        field: {
          ...
          getValue: () => {
            ...
            return {
              ...value,
              friendlyLabels: [{ period: 'Mon-Fri', description: 'All day' }]
            };
          }
        }
      };
    ...
    test('remove a friendly label', async () => {
      ...
      expect(overrideSdk.field.setValue).toHaveBeenCalledTimes(1);
      expect(overrideSdk.field.setValue.mock.results[0].value.friendlyLabels.map((l) => l.period)).not.toContain(
        'Tue/Thu'
      );
    });
  });
