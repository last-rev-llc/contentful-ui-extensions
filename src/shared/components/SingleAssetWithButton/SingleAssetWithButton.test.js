...
describe('<SingleAssetWithButton />', () => {
  test('should render the AssetCard if asset is found', async () => {
    const { debug, queryByTestId } = render(<SingleAssetWithButton sdk={sdk} assetId="1234asdf" />);
    await wait();
    expect(queryByTestId('SingleAssetWithButton-AssetCard')).toBeTruthy();
    expect(queryByTestId('SingleAssetWithButton-Button')).toBeFalsy();
  });

  test('should render the button if no asset is found', async () => {
    const { queryByTestId } = render(<SingleAssetWithButton sdk={sdk} />);
    await wait();
    expect(queryByTestId('SingleAssetWithButton-AssetCard')).toBeFalsy();
  ...
  test('should show error message if JSON is not valid', async () => {
    const { queryByTestId } = render(<SingleAssetWithButton assetId="1234asdf" sdk={sdk} />);
    await wait();
  ...
  test('should call handleFieldChange if passed in', async () => {
    ...
    const { getByTestId } = render(<SingleAssetWithButton sdk={sdk} handleFieldChange={handleFieldChange} />);
    await wait();
  ...
  test('should render asset menu when clicked on', async () => {
    const { debug, getByTestId, queryByTestId } = render(<SingleAssetWithButton assetId="1234asdf" sdk={sdk} />);
    await wait();
  ...
  test('should handleRemoveImage when remove action menu item clicked and passed in', async () => {
    ...
    const { getByTestId } = render(
      <SingleAssetWithButton assetId="assethandleRemoveImage" sdk={sdk} handleRemoveImage={handleRemoveImage} />
    );
    await wait();
    fireEvent.click(getByTestId('cf-ui-card-actions').getElementsByTagName('button')[0]);
    await wait();
  ...
  test('clicking on change image should call handleChangeImage if passed in', async () => {
    ...
    const { getByTestId } = render(
      <SingleAssetWithButton assetId="assethandleRemoveImage" sdk={sdk} handleChangeImage={handleChangeImage} />
    );
    await wait();
    fireEvent.click(getByTestId('cf-ui-card-actions').getElementsByTagName('button')[0]);
    await wait();
